 ...
 */
export class CategoryChannelGateway extends GatewayStorage {
  ...
  /**
	...
	get(id: string) {
		...
		return channel && channel.type === 'category' ? channel.settings : undefined;
	}
	 ...
	 * @returns {?(CategoryChannelGateway|external:Settings)}
	 */
	async sync(input = this.client.channels.reduce((channels, channel) => channel.type === 'category' ? channels.concat(channel.id) : channels, [])) {
		if (Array.isArray(input)) {
			...
			for (const channel of this.client.channels.values()) {
				// @ts-ignore
				if (channel.type === 'category' && !channel.settings._existsInDB) channel.settings._existsInDB = false;
			}
			return this;
		}
