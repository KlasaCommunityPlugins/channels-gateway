 ...
 */
export class VoiceChannelGateway extends GatewayStorage {
  ...
  /**
	...
	get(id: string) {
		...
		return channel && channel.type === 'voice' ? channel.settings : undefined;
	}
	 ...
	 * @returns {?(VoiceChannelGateway|external:Settings)}
	 */
	async sync(input = this.client.channels.reduce((channels, channel) => channel.type === 'voice' ? channels.concat(channel.id) : channels, [])) {
		if (Array.isArray(input)) {
			...
			for (const channel of this.client.channels.values()) {
				// @ts-ignore
				if (channel.type === 'voice' && !channel.settings._existsInDB) channel.settings._existsInDB = false;
			}
			return this;
		}
